...
from schwarm.agents.cli.decorators import cli_command, cli_param
...
async def search(
    ...
    return [
        f"Result {i} for: {query} (sort: {sort}){tags_str}"
...
async def main():
    ...
    agent = (
        ...
        .with_instructions(
            "I am an agent that can generate images, search, and summarize text."
        )
    ...
    provider = next(
        p for p in agent._providers
        if isinstance(p, CLILLMProvider)
    )
    ...
    is_valid, error = await provider.validate_command(
        command1,
        agent._functions["image-gen"]
    )
    
    if is_valid:
        result = await agent._functions["image-gen"].execute_cli(
            agent.context,
            command1
        )
    ...
    is_valid, error = await provider.validate_command(
        command2,
        agent._functions["search"]
    )
    
    if is_valid:
        result = await agent._functions["search"].execute_cli(
            agent.context,
            command2
        )
