...
class CLIFunction(Function):
    ...
    def __init__(
        ...
        """
        super().__init__(name=name, implementation=implementation, description=description)
        self.command = Command(name=name, description=description, parameters=parameters)
    ...
    async def execute_cli(self, context: Context, command_str: str) -> Any:
        ...
        if not isinstance(result, dict):
            raise ValueError(f"Expected a dictionary of parameters, got {type(result).__name__}")
    ...
    def _convert_params_to_kwargs(self, params: dict[str, str]) -> dict[str, Any]:
        ...
        return {flag.lstrip("-"): value for flag, value in params.items()}
