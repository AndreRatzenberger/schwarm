...
from schwarma.events.events import EventDispatcher, EventListener, EventType
from schwarma.functions.cli import CLIFunction, Parameter
from schwarma.functions.function import Function
from schwarma.providers.cli_llm_provider import CLILLMProvider
...
class AgentBuilder:
    ...
    def with_cli_function(
        self, name: str, implementation: Callable[..., Any], description: str, parameters: list[Parameter]
    ) -> "AgentBuilder":
        ...
        cli_function = CLIFunction(
            name=name, implementation=implementation, description=description, parameters=parameters
        )
        return self.with_function(cli_function)

    def with_cli_functions(
        self, functions: list[tuple[str, Callable[..., Any], str, list[Parameter]]]
    ) -> "AgentBuilder":
    ...
    def build(self) -> Agent:
        ...
        has_cli_functions = any(isinstance(f, CLIFunction) for f in self._functions)
        has_cli_provider = any(isinstance(p, CLILLMProvider) for p in self._providers)

        # Add CLILLMProvider if needed
        if has_cli_functions and not has_cli_provider:
            self._providers.append(CLILLMProvider(model="gpt-3.5-turbo", temperature=0.7))
